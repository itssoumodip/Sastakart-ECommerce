...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      try {
        ...
        if (token) {
          logger.debug('AuthContext: Found valid authentication token');
          ...
          logger.debug('AuthContext: No token found in any storage location');
  ...
  const loadUser = async () => {
    try {
      logger.debug('AuthContext: Loading user data...');
      ...
      logger.debug('AuthContext: User data received:', !!data?.user);
      ...
      if (data.user) {
        ...
        logger.debug('AuthContext: User loaded successfully:', data.user.email);
      } else {
        logger.warn('AuthContext: No user data in response');
        throw new Error('No user data received')
      }
    } catch (error) {
      logger.error('AuthContext: Load user error:', error.message, error.response?.status);
      // Clear token if it's an authentication error
      if (error.response?.status === 401) {
        logger.warn('AuthContext: 401 error detected - clearing auth data');
  ...
  const login = async (email, password) => {
    try {
      ...
      if (data.token) {
        logger.debug('Login successful, storing auth token');
  ...
  const googleLogin = async (credentialResponse) => {
    try {
      ...
      logger.debug('Sending Google token to backend for verification:', credentialResponse.credential);
      ...
      logger.debug('Google auth response from backend:', data);
      ...
      if (data.token) {
        ...
        logger.error('No token received from backend after Google login');
      ...
      logger.error('Google login error details:', error);
