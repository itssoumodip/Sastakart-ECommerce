...
const ProductCard = forwardRef(({ product, index = 0, viewMode = 'grid' }, ref) => {
  ...
  return (
    <motion.div
      ...
      animate="visible"
      whileHover={{ y: -4 }}
    >      <Link to={`/products/${product._id}`} className={viewMode === 'list' ? 'md:flex md:w-full' : ''}>
        <div className={`relative overflow-hidden ${
          viewMode === 'list' ? 'md:w-1/3 lg:w-1/4' : ''
        }`}>
          <img
        ...
        <div className={`p-5 ${viewMode === 'list' ? 'md:flex-1 md:flex md:flex-col' : ''}`}>
          ...
          <div className="flex items-center justify-between mt-auto">
            ...
            >
              ...
              {viewMode === 'list' && <span className="hidden md:inline">Add to Cart</span>}
            </motion.button>
          </div>
        </div>
      </Link>
